        ...
        <div className="max-w-3xl mx-auto p-4">
          ...
          <Tabs defaultActiveKey="0">
            <Tabs.Item key="0" tab="Unread">
              ...
              {user?.data.notifications?.length > 0 &&
                user.data.notifications.map((nmessage) => (
                  <div className="bg-white rounded-lg shadow-md mb-4 cursor-pointer" key={nmessage.data.doctorId} onClick={() => navigate(nmessage.onClickPath)}>
            ...
            <Tabs.Item key="1" tab="Read">
              ...
              {user?.data.seennotifications?.length > 0 && user.data.seennotifications.map((nmessage) => (
                <div className="bg-white rounded-lg shadow-md mb-4 cursor-pointer" key={nmessage.data.doctorId} onClick={() => navigate(nmessage.onClickPath)}>
                  <div className="p-4">{nmessage.message}</div>
                </div>
              ))}
